...
option_list = list(
  ...
  make_option(c("-c", "--contaminant"), type="character", default="/lustre/user/liclab/fangq/tools/MaxQuant_2.4.0.0/bin/conf/contaminants.fasta", 
...
mqpar = xmlParse("/lustre/user/liclab/fangq/tools/MaxQuant_2.4.0.0/mqpar_2.4.0.xml",encoding = 'UTF-8')
# class(mqpar)
...
for( i in 1:file_num){
  if(i==1){
    ...
    newXMLNode("string","2",parent = xmltop[['referenceChannel']])
...
XML::saveXML(xmltop,file = file.path(opt$xmlPath,"mqpar.xml"))
